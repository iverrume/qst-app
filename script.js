...
*You can enter multiple questions at once`,
    ...
    return {
        ...
        /**
         * Устанавливает язык для модуля чата и принудительно обновляет весь видимый UI.
         * @param {string} lang - Код языка ('ru', 'en', 'kz').
         */
        setLanguage: (lang) => {
            if (LANG_PACK_CHAT[lang]) {
                // 1. Устанавливаем новую языковую переменную
                currentChatLang = lang;
                localStorage.setItem('chatLanguage', lang);

                // 2. Если чат закрыт, ничего больше не делаем.
                // При следующем открытии он сам подхватит нужный язык.
                if (!chatOverlay || chatOverlay.classList.contains('hidden')) {
                    return;
                }

                // 3. Если чат ОТКРЫТ, выполняем полное обновление:
                // 3а. Обновляем статическую "оболочку" (меню, заголовки, поля ввода)
                updateChatUIText();

                // 3б. КЛЮЧЕВОЙ ФИКС: Принудительно перезагружаем и перерисовываем
                // контент текущей активной вкладки (сообщения, вопросы и т.д.)
                loadTabData(currentTab); 
            }
        },
        // === КОНЕЦ НОВОГО МЕТОДА ===
